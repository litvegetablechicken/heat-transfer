function out = annular(param, Tw_d, L, delta_in, state0)
...
if isfield(param,'annular') && isfield(param.annular,'entraintment_calculation')
    ...
    entraintment_calculation = "cal_entraintment"; % or "no_entraintment"
end
...
if ~isfield(state0,'G_L0');  state0.G_L0  = param.annular.G_L0; end
if ~isfield(state0,'G_V0');  state0.G_V0  = param.annular.G_V0; end
...
for k = 1:Nz
    ...
    % 你给的：q_r = h*(T_w - T)，T=T_BP
    % 但 h 的某些关联式需要 q_r（例如 Cooper h_nb ~ q^(0.67)），因此做局部迭代
    DT = (Tw(k) - T(k));
    C = 55.0 * ( (P/Pc)^0.12 ) * ( -log10(P/Pc) )^(-0.55) * ( Mw^(-0.5) );
    h_nb   = ( (C) * (DT)^(0.67) )^(1/0.33);
    % Cooper nucleate boiling： h_nb*1E3 = 55*(P/Pc)^0.12*(-log10(P/Pc))^-0.55*Mw^-0.5*( (q_r*1E3)^0.67 )
    % 这里按原式写：h_nb 的单位是 kW/m2K -> h_nb(SI)= (右式)/1E3
%     h_nb = (55.0*((P/Pc)^0.12)*(-log10(P/Pc))^(-0.55)*(Mw^(-0.5))*((q_it)^0.67)); % W/m2K
